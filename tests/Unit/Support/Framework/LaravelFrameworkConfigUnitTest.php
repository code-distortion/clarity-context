...
{
    /**
     * Test that the framework config object is cached.
     *
     * @test
     *
     * @return void
    ...
    public static function test_framework_config_crud(): void
    {
        ...
        $key = InternalSettings::LARAVEL_CONTEXT__CONFIG_NAME . '.enabled';

        // when the value is a string - returns null
        $config->updateConfig([$key => 'abc']);
        self::assertNull($config->getEnabled());
        ...
        $config->updateConfig([$key => 'abc']);
        self::assertSame('abc', $config->getLevelWhenKnown());

        // when the value is a string with commas - returns the string
        $config->updateConfig([$key => 'abc,def']);
        self::assertSame('abc,def', $config->getLevelWhenKnown());
        ...
        // retrieving an ARRAY
        $key = InternalSettings::LARAVEL_CONTROL__CONFIG_NAME . '.channels.when_known';

        // when the value is a string - returns an array with the string as the value
        $config->updateConfig([$key => 'abc']);
        self::assertSame(['abc'], $config->getChannelsWhenKnown());

        // when the value is a string with commas - returns an array with the string values separated by the commas
        $config->updateConfig([$key => 'abc,def']);
     ...
     *
     * @return void
     * @throws ClarityContextInitialisationException When the framework can't be resolved.
     */
